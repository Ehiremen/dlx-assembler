...
string immToBinary(const string &immediate){
    ...
    if (immDecimal < 0){
        ...
        int *binaryNum = (int *) calloc(IMM_LENGTH, sizeof(int));
        int i = IMM_LENGTH;
        while (immDecimal > 0) {
            binaryNum[i - 1] = immDecimal % 2;
            immDecimal = immDecimal / 2;
            i--;
        }

        for (int x = 0; x < IMM_LENGTH; x++) {
            immBinaryString.push_back(binaryNum[x] + '0');
        }
    }
